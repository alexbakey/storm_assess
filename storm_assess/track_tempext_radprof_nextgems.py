
""" 
Load function to read in TempestExtremes (Ullrich et al., 2017, 2021) output

"""
import os.path
import datetime
#import netcdftime
import storm_assess


def load(fh, unstructured_grid = False, calendar=None):
    """
    N.B.
    This function assumes that added fields are, in order:
        MSLP, 10m wind speed, geopotential height, sst and precipitation flux.
    """

    # allow users to pass a filename instead of a file handle.
    if isinstance(fh,str):
        with open(fh,'r') as fh:
            fh_lines = fh.readlines()
#            for data in load(fh, calendar=calendar):
#                yield data

    # create list of storm start line indices
    startline_idx = []

    # for each line in the file handle            
    for line in fh_lines:
        if line.startswith('start'):    # new storm
            startline_idx.append(fh_lines.index(line))
#            else:
#                raise ValueError('Unexpected line in TempestExtremes output file.')

    for idx in startline_idx:
        # create a new observations list
        storm_obs = []

        snbr = idx                     # snbr not output by TempestExtremes

        start_line = fh_lines[idx]
        header = start_line.split()
        n_obs = int(header[1])         # number of timesteps
        start_yr = int(header[2])
        start_mon = int(header[3])
        start_day = int(header[4])
        start_hr = int(header[5])

        # read in storm's observations
        if unstructured_grid:
            for i in range(idx+1,idx+1+n_obs):
                s_line = fh_lines[i]    # storm line
                s_obs = s_line.split()  # storm's observations

                node = int(s_obs[0])
                lon, lat = float(s_obs[1]),float(s_obs[2])
                vort = None             # dummy value
                mslp = float(s_obs[3])/100.
                vmax = float(s_obs[4])
                zg250max = float(s_obs[5])
                zg250min = float(s_obs[6])
                zg500max = float(s_obs[7])
                zg500min = float(s_obs[8])
                sst = float(s_obs[9])
                tp = float(s_obs[10])
                rprof = [float(i) for i in s_obs[11].rsplit('"')[1].rsplit('[')[1].rsplit(']')[0].rsplit(',')]
                r_8 = float(s_obs[12])
                r_17 = float(s_obs[13])
                ike_8 = float(s_obs[14])
                ike_17 = float(s_obs[15])
                yr, mon, day, hr = int(s_obs[16]),int(s_obs[17]),int(s_obs[18]),int(s_obs[19])
                timestamp = datetime.datetime(yr,mon,day,hr)

                # store observations
                storm_obs.append(storm_assess.Observation(timestamp, lat, lon, vort, vmax, mslp,
                    extras = {
                        'zg250max':zg250max,
                        'zg250min':zg250min,
                        'zg500max':zg500max,
                        'zg500min':zg500min,
                        'sst':sst,
                        'tp':tp,
                        'rprof':rprof,
                        'r_8':r_8,
                        'r_17':r_17,
                        'ike_8':ike_8,
                        'ike_17':ike_17}))

            # yield storm
            yield storm_assess.Storm(snbr, storm_obs, extras={})

        elif not unstructured_grid:
            for i in range(idx+1,idx+1+n_obs):
                s_line = fh_lines[i]    # storm line
                s_obs = s_line.split()  # storm's observations

                x, y = int(s_obs[0]),int(s_obs[1])
                lon, lat = float(s_obs[2]),float(s_obs[3])
                vort = None             # dummy value
                mslp = float(s_obs[4])/100.
                vmax = float(s_obs[5])
                zg250max = float(s_obs[6])
                zg250min = float(s_obs[7])
                zg500max = float(s_obs[8])
                zg500min = float(s_obs[9])
                sst = float(s_obs[10])
                tp = float(s_obs[11])
                rprof = [float(i) for i in s_obs[12].rsplit('"')[1].rsplit('[')[1].rsplit(']')[0].rsplit(',')]
                r_8 = float(s_obs[13])
                r_17 = float(s_obs[14])
                ike_8 = float(s_obs[15])
                ike_17 = float(s_obs[16])
                yr, mon, day, hr = int(s_obs[17]),int(s_obs[18]),int(s_obs[19]),int(s_obs[20])
                timestamp = datetime.datetime(yr,mon,day,hr)

                # store observations
                storm_obs.append(storm_assess.Observation(timestamp, lat, lon, vort, vmax, mslp,
                    extras = {
                        'zg250max':zg250max,
                        'zg250min':zg250min,
                        'zg500max':zg500max,
                        'zg500min':zg500min,
                        'sst':sst,
                        'tp':tp,
                        'rprof':rprof,
                        'r_8':r_8,
                        'r_17':r_17,
                        'ike_8':ike_8,
                        'ike_17':ike_17}))

            # yield storm
            yield storm_assess.Storm(snbr, storm_obs, extras={})


if __name__ == '__main__':

    pass

